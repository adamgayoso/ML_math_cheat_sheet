...
\subsubsection{$d(\Trace(\bmx \bmx^\top \bmA)) / d\bmx$}
\begin{align*}
d(\Trace(\bmx \bmx^\top \bmA)) / d\bmx &= \frac{d}{d \bmx} \sum_{i}^{N} \sum_{k}^{N} x_i x_k a_{ik}.
\end{align*}
Consider
\begin{align*}
\frac{d}{d x_j} \sum_{i}^{N} \sum_{k}^{N} x_i x_k a_{ik} & = x_1a_{j,1} + x_2a_{j,2} + \cdots \frac{d}{d x_j} \sum_{k}^{N} x_j x_k a_{jk} + \cdots x_N a_{j, N}
\end{align*}
